...
select r.rating , m.title , u.user_id
from ratings r
join movies m 
on m.movie_id=r.movie_id
join users u
...
select a.name, count(movie_id)as TotalMovies
from actors a
...
select u.name , u.user_id,round(avg(r.rating),2)as Avg_rating 
from ratings r
...
RANK() OVER(order by release_year) as Release_rank
from movies;
...
select m.title as Name , m.release_year as Release_Year , g.genre_name as Genre, a.name as Actor, r.rating as rating
from movies m
